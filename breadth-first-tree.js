...
Tree.prototype.BFSelect = function(filter) {
  ...
  while(check < treeArray.length) {
    children = treeArray[check].children
    for (var i = 0; i < children.length; i++) {
      treeArray.push(children[i])
      depth.push(depth[check] + 1);
    }
    check++;
  }

  for (i = 0; i < treeArray.length; i++) {
    if (filter(treeArray[i].value, depth[i])) {
      output.push(treeArray[i].value);
    }
  }
